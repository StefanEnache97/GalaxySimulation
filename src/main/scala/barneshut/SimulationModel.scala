...
class SimulationModel {
  ...
  def initialize(parallelismLevel: Int, pattern: String, totalBodies: Int): Unit = {
    ...
    pattern match {
      ...
      case "one-galaxy" => init1Galaxy(totalBodies)
      case _ => sys.error(s"no such initial pattern: $pattern")
    }
  }

  def init1Galaxy(totalBodies: Int): Unit = {
    ...
    def galaxy(from: Int, num: Int, maxradius: Float, cx: Float, cy: Float, sx: Float, sy: Float): Unit = {
      ...
      for (i <- from until (from + num)) {
        val b = if (i == from) {
          ...
          val exponential_radius = math.log(1 - random.nextFloat).toFloat/(-2/maxradius)/random.nextFloat
          //val radius = 25 + maxradius * random.nextFloat
          val starx = 25 + cx + exponential_radius * math.sin(angle).toFloat
          val stary = 25 + cy + exponential_radius * math.cos(angle).toFloat
          val speed = math.sqrt(gee * blackHoleM / exponential_radius + gee * totalM * exponential_radius * exponential_radius / cubmaxradius)
          //val speed = 100.0f
    ...
    galaxy(0, bodyArray.length, 300.0f, galaxy_centre._1, galaxy_centre._2, 0.0f, 0.0f)
