...
import FloatOps._
  ...
  @Test def `Fork with 4 empty quadrants`: Unit = {
    ...
    assert(quad.massX ~= 20f, s"${quad.massX} should be 18f")
    assert(quad.massY ~= 30f, s"${quad.massY} should be 26f")
  ...
  @Test def `Leaf.insert(b) should return a new Fork if size > minimumSize`: Unit = {
    val b1 = new Body(123f, 18f, 26f, 0f, 0f)
    val b2 = new Body(524.5f, 24.5f, 25.5f, 0f, 0f)
    val b3 = new Body(245f, 22.4f, 41f, 0f, 0f)
    val quad = Leaf(15f, 30f, 20f, Seq(b2))
    val new_fork = quad.insert(b1)
    println((new_fork))
  }
  ...
  @Test def `'SectorMatrix.+=' should add a body at (25,47) to the correct bucket of a sector matrix of size 100 (2pts)`: Unit = {
    val body = new Body(5, 25, 47, 0.1f, 0.1f)
    val boundaries = new Boundaries()
    boundaries.minX = 1
    boundaries.minY = 1
  ...
  @Test def `'SectorMatrix.+=' should add a body at (64,27) to the correct bucket of a sector matrix of size 108 (2pts)`: Unit = {
